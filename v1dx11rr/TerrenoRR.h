...
private:
	...
	ID3D11InputLayout* inputLayout02;
	ID3D11Buffer* vertexBuffer02;
	ID3D11Buffer* indexBuffer02;

	ID3D11ShaderResourceView* colorMap;
	ID3D11ShaderResourceView* colorMap2;
	ID3D11ShaderResourceView* TexturaAsfalto;
	ID3D11ShaderResourceView* TexturaAsfaltoNormal;
	ID3D11ShaderResourceView* TexturaCesped;
	ID3D11ShaderResourceView* TexturaCespedNormal;
	ID3D11ShaderResourceView* TexturaBanqueta;
	ID3D11ShaderResourceView* TexturaBanquetaNormal;
	...
	ID3D11Buffer* waterCB;

	ID3D11ShaderResourceView* colorWater;
...
public:
	TerrenoRR(int ancho, int alto, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, WCHAR* texturaTerr, WCHAR* heightmapTerr, bool esAgua)
	{
		...
		if (esAgua == false) {
			CargaParametros(texturaTerr, heightmapTerr, 10.0f);
		}
		else {
			CargaParametrosWater(texturaTerr, heightmapTerr, 10.0f);
		}
	}
	...
	bool CargaParametros(WCHAR* diffuseTex, WCHAR* heightTex, float tile)
	{
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/MontañaNormal.png", 0, 0, &colorMap2, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/Asfalto.jpg", 0, 0, &TexturaAsfalto, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/AsfaltoNormal.jpg", 0, 0, &TexturaAsfaltoNormal , 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/Cesped.jpg", 0, 0, &TexturaCesped, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/CespedNormal.jpg", 0, 0, &TexturaCespedNormal, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/Pavimento.jpg", 0, 0, &TexturaBanqueta, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/PavimentoNormal.jpg", 0, 0, &TexturaBanquetaNormal, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Imagenes/TexturasTerreno/blendingmap02.png", 0, 0, &blendMap, 0 );
	...
	bool CargaParametrosWater(WCHAR* diffuseTex, WCHAR* heightTex, float tile)
	{
		HRESULT d3dResult;
		//carga el mapa de alturas
		LoadHeightData(heightTex);
		...
		//que VS_Main es el nombre del vertex shader en el shader, vsBuffer contendra
		//al puntero del mismo
		bool compileResult = CompileD3DShader(L"ShaderWater.fx", "VS_Main", "vs_4_0", &vsBuffer);
		//en caso de no poder cargarse ahi muere la cosa
		if (compileResult == false)
		{
			return false;
		...
		// de los vertices pasamos al pixel shader, note que el nombre del shader es el mismo
		//ahora buscamos al pixel shader llamado PS_Main
		compileResult = CompileD3DShader(L"ShaderWater.fx", "PS_Main", "ps_4_0", &psBuffer);
		...
		//crea los accesos de las texturas para los shaders 
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, diffuseTex, 0, 0, &colorMap, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, L"Imagenes/TexturasAgua/foam02.png", 0, 0, &colorMap2, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, L"Imagenes/TexturasAgua/water_normal.png", 0, 0, &blendMap, 0);
		...
		//de mundo
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &waterCB);
	...
	bool UnloadContent()
	{
		...
		if(inputLayout)
			inputLayout->Release();
		if (inputLayout02)
			inputLayout02->Release();
		if(vertexBuffer)
			vertexBuffer->Release();
		if (vertexBuffer02)
			vertexBuffer02->Release();
		if(viewCB)
			viewCB->Release();
		if(projCB)
			projCB->Release();
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXMATRIX water, bool esagua)
	{
		...
		if (esagua == false) {
			...
			d3dContext->PSSetShaderResources(2, 1, &TexturaAsfalto);
			d3dContext->PSSetShaderResources(3, 1, &TexturaAsfaltoNormal);
			d3dContext->PSSetShaderResources(4, 1, &TexturaCesped);
			d3dContext->PSSetShaderResources(5, 1, &TexturaCespedNormal);
			d3dContext->PSSetShaderResources(6, 1, &TexturaBanqueta);
			d3dContext->PSSetShaderResources(7, 1, &TexturaBanquetaNormal);
			d3dContext->PSSetShaderResources(8, 1, &blendMap);
		...
		if (esagua == true) {
			d3dContext->UpdateSubresource(waterCB, 0, 0, &water, 0, 0);
		}
		...
		if (esagua == true) {
			d3dContext->VSSetConstantBuffers(3, 1, &waterCB);
		}
	...
	//	//define la estructura del vertice a traves de layout
	//	d3dContext->IASetInputLayout(inputLayout02);

	//	//define con que buffer trabajara
	//	d3dContext->IASetVertexBuffers(0, 1, &vertexBuffer02, &stride, &offset);
	//	//define con buffer de indices trabajara
	//	d3dContext->IASetIndexBuffer(indexBuffer, DXGI_FORMAT_R32_UINT, 0);
	//	//define la forma de conexion de los vertices
	//	d3dContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
