...
public:
	ModeloRR(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, 
		WCHAR* specularTexturePath, float _posX, float _posZ)
	{
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, float posX, float posY,float posZ, float rotX, float rotY, float rotZ)
	{
		static float rotation = 0.0f;
		rotation += 0.01;
		...
		if (angle == 'X')
			D3DXMatrixRotationX(&rotationMat, rotX);
		else if (angle == 'Y')
			D3DXMatrixRotationY(&rotationMat, rot);
		else if (angle == 'Z')
			D3DXMatrixRotationZ(&rotationMat, rotZ);
		viewMatrix *= rotationMat;
